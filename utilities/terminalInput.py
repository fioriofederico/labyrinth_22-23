...
class terminalInput:
    ...
    def main_menu(self):
        while True:
            try:
                ...
                if choice == 1:
                    self.create_maze(menu)
                elif choice == 2:
                    self.upload_image(menu)
                elif choice == 3:
                    self.upload_json(menu)
                else:
    ...
    def create_maze(self, menu):
        ...
        while True:
            ...
            if add_start == 'no' or add_start == 'n':
                break
            start = list(map(int, input("Insert the start point as a list: ").split()))
            startPoint.append(start)
    ...
    def check_file_extension(self, file_path, extension):
    ...
    def upload_image(self, menu):
        ...
        while True:
            ...
            if self.check_file_extension(path, '.tiff'):
